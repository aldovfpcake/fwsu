...
FOR I = 1 TO 11
 ...
 FOR x = 1 TO  12
    ...
    * SELECT SUM(aporte) as aporte ,SUM(sinaporte) as sinaporte,SUM(IIF(CONCEPTO =17 .OR. CONCEPTO = 23,APORTE,0))AS KM,SUM(IIF(CONCEPTO =14,APORTE,0))AS ctrld,;
     ...
     SELECT SUM(aporte) as aporte ,SUM(sinaporte) as sinaporte,SUM(IIF(CONCEPTO =830,descuento,0))AS KM,SUM(IIF(CONCEPTO =870,descuento,0))AS ctrld,;
     SUM(DESCUENTO)as descuento,liquida FROM &archivo WHERE legajo = vvlegajo  GROUP BY LIQUIDA INTO CURSOR liquida
     
   *  SELECT SUM(aporte) as aporte ,SUM(sinaporte) as sinaporte,SUM(IIF(CONCEPTO =130 .OR. CONCEPTO =99,0,DESCUENTO))as descuento,;
     SUM(IIF(CONCEPTO =830,DESCUENTO,0))as ctrl,SUM(IIF(CONCEPTO =870,DESCUENTO,0))as km liquida FROM &archivo WHERE legajo = vvlegajo .AND. LIQUIDA = 3 GROUP BY LIQUIDA INTO CURSOR liquida

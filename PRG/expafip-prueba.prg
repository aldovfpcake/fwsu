...
*SET FILTER TO CHAPA = 795 .OR. CHAPA = 875 .OR. CHAPA = 835
SELECT sue3
SCAN 
s.cuil = cuil
s.setnombre(nombre)
...
s.Setactividad(ACTIVIDAD)
 ...
 SELECT sue3
  ...
  + STr(sbruto,12,2)+STr(sbruto,12,2)+"0"+STR(CODSINIEST,1)+"0"+"000000.00"+tipoempresa+aporteadicionalobrs+"1"+PADL(ALLTRIM(tablaper.situaret),2,"00")+PADL(ALLTRIM(STR(TABLAPER.diani1,2)),2,"00");
  ...
  +STR(premios,12,2)+STr(sbruto,12,2)+"000000000.00"+PADL(ALLTRIM(STR(canthoras,3)),3,"0")+STR(SVIATICO,12,2)+"000000000.00"+"000000.00"+STR(sbruto,12,2);

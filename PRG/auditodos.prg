...
SUM(IIF(CONCEPTO = 83,1,0))AS SIND,SUM(IIF(CONCEPTO = 59 .OR. CONCEPTO = 64,SINAPORTE,0))AS SINAP,SUM(IIF(INLIST(CONCEPTO,1,450),APORTE,0)) AS BASICO,SUM(IIF(CONCEPTO =450,SINAPORTE,0)) AS BS ;
FROM 32021 WHERE LIQUIDA = 3  GROUP BY LEGAJO INTO CURSOR ONE READWRITE
SCAN
    IF BS <> 0
       REPLACE BASICO WITH BS
    ENDIF   
...
*
SELECT P.LEGAJO,P.FECHAING,P.NOMBRE,P.CUIL, P.DEPART,P.SUELDOFIJO,P.CATEGORIA FROM PERSONAL AS P WHERE ACTIVO = "A" .AND. FECHAING < CTOD("01-01-2021") INTO CURSOR PACTIVO

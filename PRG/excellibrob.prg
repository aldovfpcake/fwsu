...
SELECT LEGAJO,CONCEPTO,CANTIDAD,SUM(APORTE)AS APORTE ,SUM(SINAPORTE) AS SINAPORTE, SUM(DESCUENTO) AS DESCUENTO FROM B42021 WHERE LEGAJO =VarLegajo   GROUP BY LEGAJO,CONCEPTO,CANTIDAD ORDER BY CONCEPTO INTO CURSOR SUELDO READWRITE
fila = 5
...
SCAN
   ...
   ENDIF
   IF sueldo.concepto = 402
      oExcel.cells(fila,4) = 4402
   ELSE       
     oExcel.cells(fila,4) = sueldo.concepto
   ENDIF
   ...
   IF sueldo.aporte <> 0
      ...
      IF sueldo.concepto = 4 .OR. sueldo.concepto = 7 .OR. concepto = 22
